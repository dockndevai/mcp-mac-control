...
export class MacClient {
  ...
  /** Press at (x1,y1), move to (x2,y2), release — a human-style drag. */
  async dragMouse(x1: number, y1: number, x2: number, y2: number): Promise<void> {
    await this.requireCliclick();
    const r = await this.run("cliclick", [`dd:${x1},${y1}`, `m:${x2},${y2}`, `du:${x2},${y2}`]);
    if (r.code !== 0) throw new MacError(`cliclick drag failed: ${r.stderr.trim() || `exit ${r.code}`}`);
  }

  /**
   * Scroll the wheel via a CoreGraphics event (JXA/ObjC — no extra binary needed).
   * Uses literal CG constants (unit=line=1, tap=0) so no framework constant lookup is required.
   */
  async scroll(direction: "up" | "down" | "left" | "right", amount: number): Promise<void> {
    const lines = Math.max(1, Math.min(100, Math.floor(amount || 3)));
    const vy = direction === "up" ? lines : direction === "down" ? -lines : 0;
    const vx = direction === "left" ? lines : direction === "right" ? -lines : 0;
    const js =
      "ObjC.import('CoreGraphics');" +
      `(function(){var e=$.CGEventCreateScrollWheelEvent($(),1,2,${vy},${vx});$.CGEventPost(0,e);})();`;
    await this.osa(js, "JavaScript");
  }

  // ---- perceive the desktop ----
  /** Main-screen size in points. */
  async screenSize(): Promise<{ width: number; height: number }> {
    const out = await this.osa('tell application "Finder" to get bounds of window of desktop', "AppleScript", 12_000);
    const n = out.split(",").map((s) => parseInt(s.trim(), 10));
    return { width: n[2] ?? 0, height: n[3] ?? 0 };
  }

  /** Windows of the frontmost app, with name/position/size — so a caller can click reliably. */
  async listWindows(): Promise<Array<{ name: string; position: string; size: string }>> {
    const script = [
      'tell application "System Events" to tell (first process whose frontmost is true)',
      "set res to \"\"",
      "repeat with w in windows",
      'set res to res & name of w & "\\t" & (position of w as text) & "\\t" & (size of w as text) & "\\n"',
      "end repeat",
      "return res",
      "end tell",
    ].join("\n");
    const out = await this.osa(script, "AppleScript", 12_000);
    return out
      .split("\n")
      .filter(Boolean)
      .map((l) => {
        const [name, position, size] = l.split("\t");
        return { name: name ?? "", position: position ?? "", size: size ?? "" };
      });
  }

  // ---- app / window control ----
  async activateApp(name: string): Promise<void> {
    await this.osaArgv(["on run argv", "activate application (item 1 of argv)", "end run"], [name], 12_000);
  }
  async quitApp(name: string): Promise<void> {
    await this.osaArgv(["on run argv", "tell application (item 1 of argv) to quit", "end run"], [name], 12_000);
  }
}
