 ...
 * MACCTL_ALLOW_EXEC; deleting needs MACCTL_ALLOW_DELETE; GUI input needs MACCTL_ALLOW_INPUT. The exec,
...
export const adminTools: ToolDef[] = [
  {
    ...
    config: {
      ...
      description:
        ...
        "MACCTL_ALLOW_EXEC=true; the program must be in MACCTL_COMMAND_ALLOWLIST when that is set. For a shell " +
  ...
  },
  {
    ...
    config: {
      ...
      description:
        ...
        "scriptable app. Requires admin mode AND MACCTL_ALLOW_EXEC=true.",
      inputSchema: {
  ...
  },
  {
    ...
    config: {
      ...
      description:
        "Move a file or folder to the Trash (recoverable). Requires admin mode AND MACCTL_ALLOW_DELETE=true. " +
  ...
  },
  {
    ...
    config: {
      title: "Type text",
      description:
        "Type literal text into the frontmost app (keystrokes). Requires admin mode AND MACCTL_ALLOW_INPUT=true, " +
  ...
  },
  {
    ...
    config: {
      ...
      description:
        ...
        "(command/option/control/shift). Requires admin mode AND MACCTL_ALLOW_INPUT=true.",
      inputSchema: {
  ...
  },
  {
    ...
    config: {
      title: "Click",
      description:
        "Click at screen coordinates. Requires admin mode AND MACCTL_ALLOW_INPUT=true, and the `cliclick` tool " +
  ...
  },
  {
    ...
    config: {
      ...
      description: "Move the cursor to screen coordinates. Requires admin mode AND MACCTL_ALLOW_INPUT=true (cliclick).",
      inputSchema: {
        x: z.number().int().describe("X coordinate (screen points)."),
        y: z.number().int().describe("Y coordinate (screen points)."),
      },
  ...
  },
  {
    ...
    config: {
      title: "Drag",
      description:
        "Press at (x1,y1), move to (x2,y2), and release — a human-style drag (select text, move a window, " +
        "drag a file). Requires admin mode AND MACCTL_ALLOW_INPUT=true, and the `cliclick` tool.",
      inputSchema: {
        x1: z.number().int().describe("Start X (screen points)."),
        y1: z.number().int().describe("Start Y (screen points)."),
        x2: z.number().int().describe("End X (screen points)."),
        y2: z.number().int().describe("End Y (screen points)."),
      },
    },
    handler: async (args, { client, policy }) => {
      const { dryRun } = policy.guard({ tool: "drag", capability: "admin", requiresInput: true });
      if (dryRun) return textResult("[dry-run] Would drag.");
      await client.dragMouse(args.x1 as number, args.y1 as number, args.x2 as number, args.y2 as number);
      return jsonResult({ dragged: { from: [args.x1, args.y1], to: [args.x2, args.y2] } });
    },
  },
  {
    name: "scroll",
    ...
    config: {
      title: "Scroll",
      description:
        "Scroll the mouse wheel in a direction by a number of lines (over whatever is under the cursor / " +
        "frontmost). Requires admin mode AND MACCTL_ALLOW_INPUT=true.",
      inputSchema: {
        direction: z.enum(["up", "down", "left", "right"]).describe("Scroll direction."),
        amount: z.number().int().min(1).max(100).default(3).describe("Number of scroll lines (1–100)."),
      },
    },
    handler: async (args, { client, policy }) => {
      const { dryRun } = policy.guard({ tool: "scroll", capability: "admin", requiresInput: true });
      if (dryRun) return textResult("[dry-run] Would scroll.");
      await client.scroll(args.direction as "up" | "down" | "left" | "right", (args.amount as number) ?? 3);
      return jsonResult({ scrolled: { direction: args.direction, amount: args.amount ?? 3 } });
    },
  },
];
