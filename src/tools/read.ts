...
export const readTools: ToolDef[] = [
  ...
  },
  {
    ...
    config: {
      title: "Read a file",
      description: "Read a text file's contents (capped at MACCTL_MAX_FILE_BYTES). Honours the path allowlist.",
  ...
  },
  {
    ...
    config: {
      title: "List windows",
      description:
        "List the windows of the frontmost app with each window's name, position and size — so you can click " +
        "or drag reliably. Needs Accessibility permission for the host process.",
      inputSchema: {},
    },
    handler: async (_args, { client, policy }) => {
      policy.guard({ tool: "list_windows", capability: "read" });
      return jsonResult(await client.listWindows());
    },
  },
];
