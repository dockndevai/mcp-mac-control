...
export const writeTools: ToolDef[] = [
  ...
  },
  {
    ...
    capability: "write",
    config: {
      title: "Activate (focus) an app",
      description:
        "Bring an application to the front (launching it if needed) so subsequent clicks/keystrokes go to it. " +
        "Give the app's name, e.g. 'Safari', 'Notes'.",
      inputSchema: { name: z.string().min(1).describe("Application name to focus.") },
    },
    handler: async (args, { client, policy }) => {
      const name = args.name as string;
      const { dryRun } = policy.guard({ tool: "activate_app", capability: "write" });
      if (dryRun) return textResult(`[dry-run] Would activate '${name}'.`);
      await client.activateApp(name);
      return jsonResult({ activated: name });
    },
  },
  {
    name: "quit_app",
    ...
    config: {
      ...
      inputSchema: { name: z.string().min(1).describe("Application name to quit.") },
    },
    handler: async (args, { client, policy }) => {
      const name = args.name as string;
      const { dryRun } = policy.guard({ tool: "quit_app", capability: "write" });
      if (dryRun) return textResult(`[dry-run] Would quit '${name}'.`);
      await client.quitApp(name);
      return jsonResult({ quit: name });
    },
  },
];
